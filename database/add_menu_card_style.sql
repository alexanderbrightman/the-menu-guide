...
add column if not exists menu_card_style text default 'minimal'
check (menu_card_style = any (array['classic'::text, 'minimal'::text]));

comment on column profiles.menu_card_style is 'Menu item card shape: minimal = rounded (default), classic = square bordered';
